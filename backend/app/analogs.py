...
first round usually settles it), out-of-stock candidates are dropped (only_in_stock), and each candidate is scored:
  ...
  score = 0.45 * spec_match + 0.25 * type_overlap + 0.12 * category + 0.03 * same_brand + 0.05 * price_proximity + 0.10 * recommended
  spec_match   - weighted share of the base product's key specs (current, poles, leakage, section, cores, power,
                 colour temperature, IP, voltage, breaking capacity, lumen, modules) that the candidate matches;
                 unknown on the candidate side gives quarter credit; a mismatch on a critical spec (current, poles,
                 leakage, section, cores, power, modules) multiplies the final score by 0.35
  type_overlap - share of the base name's type words (non-numeric tokens minus brand/stop words) found in the candidate
Specs are parsed from the normalized name first ('160а', '3p', '1p+n', '3x2.5', '30w', '4000k', 'ip65', '30ма')
...
MAX_CANDIDATES = 48
BATCH = 16  # details fetched per round; we stop early once `limit` strong analogs are in hand
MIN_SCORE = 0.2
STRONG_SCORE = 0.45
...
_TYPE_CANON = {
    ...
    "авт": "автомат", "ав": "автомат", "ва": "автомат", "автоматический": "автомат", "автомат": "автомат",
...
def _spec_equal(key: str, a: Any, b: Any) -> bool:
    if key == "poles":
        return str(a) == str(b)
    tol = 0.12 if key in ("power", "lumen") else 0.001
...
def score_candidate(base: dict[str, Any], cand: dict[str, Any]) -> tuple[float, dict[str, Any]]:
    ...
    type_score = len(common) / max(1, len(base["types"]))
    ...
    score = 0.45 * spec_score + 0.25 * type_score + 0.12 * cat_score + 0.03 * same_brand + 0.05 * price_score + 0.10 * cand.get("recommended", 0)
...
def build_reason(base: dict[str, Any], cand: dict[str, Any], facts: dict[str, Any], quantity: int | None) -> str:
    ...
    parts: list[str] = []
    type_txt = _type_label(facts["common_types"]) or cand.get("obyem") or _type_label(cand["types"][:3])
    same = [_spec_text(k, base["specs"][k]) for k in facts["matched"]]
    head = f"Тот же тип: {type_txt}" if (facts["common_types"] or cand.get("obyem")) else f"Похожий товар ({type_txt})" if type_txt else "Похожий товар"
...
def _profile(row: dict[str, Any], detail: dict[str, Any] | None, recommended: bool = False) -> dict[str, Any]:
    ...
    return {"specs": extract_specs(name, props), "types": type_words(name, brand), "brand": brand, "price": price,
            ...
            "obyem": str(props.get("OBYEM") or "").strip() or None, "recommended": recommended}
...
async def find_analogs(product_id: int, limit: int = 3, *, only_in_stock: bool = True) -> list[dict[str, Any]]:
    ...
    # candidate ids in priority order: type words + key specs search, site recommendations, type-only search, same category
    cands: dict[int, bool] = {}  # id -> recommended by the site
    words = base["types"][:4]
    spec_tokens = [t for t in catalog.normalize_text(row["name"]).split() if any(c.isdigit() for c in t) and not t.isdigit()][:2]
    ...
    for start in range(0, len(cand_ids), BATCH):
        ...
        for cid in batch:
            ...
            if score < MIN_SCORE or not (facts["common_types"] or facts["cat_score"] or prof["recommended"]):
                continue
