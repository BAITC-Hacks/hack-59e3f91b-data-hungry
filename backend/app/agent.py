...
from .product_standards import extract_standard_context
...
- ГОСТ и ТР: когда отвечаешь о товаре, добавь короткую фразу о стандартах из его названия, характеристик, описания или относящегося к нему документа. Поле standards_context сохраняет такие фрагменты целиком перед сокращением карточки. Если источник сообщает, что товар соответствует ГОСТ/ТР, изготовлен по нему или имеет положительный флаг ГОСТ, говори «По данным карточки ЕКТ, товар соответствует ...»; для вложения — «По предоставленному документу ...». Указывай только обозначения из источника. При «ГОСТ: Да» без номера скажи «В карточке отмечено соответствие ГОСТ, конкретный номер не указан».
- Сохраняй область утверждения: стандарт маркировки, жил кабеля или условий питания относится только к указанной части/условию. Требование клиента «нужен по ГОСТ», стандарт другого товара, общее описание компании, отрицание («ГОСТ: Нет», «соответствие не подтверждено») и неполный фрагмент (truncated=true) не подтверждают соответствие товара. Если есть только упоминание, скажи «В описании указан ...» с контекстом; при противоречии явно сообщи его. Не переноси стандарты между товарами.
- Документ о сертификации и упоминание стандарта — разные сведения. Соответствие по данным карточки можно сообщить без файла сертификата. Не утверждай, что выдан сертификат/декларация, и не придумывай его номер, срок или ссылку. Если клиент просит сам документ, а проверенного файла/ссылки нет, предложи запросить его у менеджера.
...
def _card_for_llm(card: dict[str, Any], *, full: bool = False) -> dict[str, Any]:
    ...
    standard_context = extract_standard_context(card)
    if standard_context:
        out["standards_context"] = standard_context
